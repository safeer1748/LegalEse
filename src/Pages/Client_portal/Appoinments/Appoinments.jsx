...
const Appoinments = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8000/appoinments?clientId=${userId}`)
      .then((res) => {
        setData(res.data);
      })
            ...
            {data.map((d, i) => (
              ...
              >
                ...
                <div className="flex gap-8 text-sm">
                  <span className="flex flex-col md:flex-row gap-1"><label className="font-medium">Date:</label> <label className="text-gray-600">{d.date}</label></span>
                  <span className="flex flex-col md:flex-row gap-1"><label className="font-medium">Time:</label> <label className="text-gray-600">{d.time}</label></span>
                  <span className="flex flex-col md:flex-row gap-1"><label className="font-medium">Status:</label> <label className="text-gray-600">{d.status}</label></span>
                </div>
                <div className="flex">
                  <button
                   ...
                   disabled={d.status==='closed'}
                    type="button"
                    className={`${d.status==='closed'?'bg-gray-400':'bg-blue-700 hover:bg-blue-800 cursor-pointer'} text-white text-xs inline-flex items-center  px-3 py-1 rounded`}
                  >
                    Cancel Appoinment
                  </button>
                </div>
